...
async function seedDatabase() {
    try {
        ...
        const { mockUsers, mockTransactions, mockPaymentLinks, mockSupportTickets, mockWalletTransactions } = generateMockData();
        // Add admin user
        ...
        for (let batchIndex = 0; batchIndex < userBatches.length; batchIndex++) {
            ...
            userBatches[batchIndex].forEach((user, index) => {
                ...
                batch.set(docRef, Object.assign(Object.assign({}, user), { createdAt: admin.firestore.Timestamp.fromDate(user.createdAt), updatedAt: admin.firestore.Timestamp.fromDate(user.updatedAt), registrationDate: admin.firestore.Timestamp.fromDate(user.registrationDate), lastLoginDate: admin.firestore.Timestamp.fromDate(user.lastLoginDate) }));
            });
            await batch.commit();
        }
        ...
        mockTransactions.forEach((transaction, index) => {
            ...
            transactionBatch.set(docRef, Object.assign(Object.assign({}, transaction), { createdAt: admin.firestore.Timestamp.fromDate(transaction.createdAt), completedAt: transaction.completedAt ? admin.firestore.Timestamp.fromDate(transaction.completedAt) : null }));
        });
        ...
        mockPaymentLinks.forEach((link, index) => {
            ...
            paymentLinksBatch.set(docRef, Object.assign(Object.assign({}, link), { createdAt: admin.firestore.Timestamp.fromDate(link.createdAt), expiresAt: admin.firestore.Timestamp.fromDate(link.expiresAt) }));
        });
        ...
        mockSupportTickets.forEach((ticket, index) => {
            ...
            supportBatch.set(docRef, Object.assign(Object.assign({}, ticket), { createdAt: admin.firestore.Timestamp.fromDate(ticket.createdAt), repliedAt: ticket.repliedAt ? admin.firestore.Timestamp.fromDate(ticket.repliedAt) : null }));
        });
        ...
        mockWalletTransactions.forEach((transaction, index) => {
            ...
            walletBatch.set(docRef, Object.assign(Object.assign({}, transaction), { createdAt: admin.firestore.Timestamp.fromDate(transaction.createdAt) }));
        });
